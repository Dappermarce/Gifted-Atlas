...
export default function AdvancedAssessment() {
  ...
  const advancedQuestions = lang === 'es'
    ? [
        ...
        { id: 6, type: "Neurociencia", question: "Un estudio de neuroimagen encuentra una diferencia promedio entre grupos. ¿Qué conclusión es responsable?", options: ["Permite identificar a cualquier individuo", "Describe una asociación grupal que debe replicarse", "Demuestra una causa única", "Sustituye la evaluación educativa"], correct: "Describe una asociación grupal que debe replicarse", explanation: "Una diferencia promedio puede orientar nuevas preguntas, pero no funciona como marcador individual ni demuestra por sí sola causalidad." },
        { id: 7, type: "Decisión Educativa", question: "Un estudiante obtiene una puntuación alta, pero su desempeño varía entre áreas. ¿Cuál es el siguiente paso más razonable?", options: ["Aplicar la misma respuesta en todas las materias", "Ignorar el contexto", "Revisar perfil, necesidades, oportunidades y varias fuentes", "Convertir la puntuación en una etiqueta permanente"], correct: "Revisar perfil, necesidades, oportunidades y varias fuentes", explanation: "Las decisiones educativas deben responder al perfil completo y a las necesidades observadas, no a una sola cifra." },
        { id: 8, type: "Lectura Crítica", question: "Dos modelos de altas capacidades usan definiciones distintas. ¿Cómo conviene compararlos?", options: ["Elegir el más famoso", "Revisar qué miden, para qué sirven y cuáles son sus límites", "Suponer que uno invalida toda la evidencia del otro", "Combinar sus términos como si fueran equivalentes"], correct: "Revisar qué miden, para qué sirven y cuáles son sus límites", explanation: "Los modelos pueden responder preguntas diferentes. Comparar definiciones, medidas, contexto y límites evita tratarlos como etiquetas intercambiables." },
      ]
    : [
        ...
        { id: 6, type: "Neuroscience", question: "A neuroimaging study finds an average difference between groups. Which conclusion is responsible?", options: ["It can identify any individual", "It describes a group association that should be replicated", "It proves one unique cause", "It replaces educational assessment"], correct: "It describes a group association that should be replicated", explanation: "An average difference may guide further questions, but it is not an individual marker and does not establish causality by itself." },
        { id: 7, type: "Educational Decision", question: "A student earns a high score but performance varies across domains. What is the most reasonable next step?", options: ["Use the same response in every subject", "Ignore the context", "Review the profile, needs, opportunities, and several sources", "Turn the score into a permanent label"], correct: "Review the profile, needs, opportunities, and several sources", explanation: "Educational decisions should respond to the full profile and observed needs, not one score." },
        { id: 8, type: "Critical Reading", question: "Two giftedness models use different definitions. How should they be compared?", options: ["Choose the most famous one", "Review what they measure, what they are for, and their limits", "Assume one invalidates all evidence from the other", "Combine their terms as if they were equivalent"], correct: "Review what they measure, what they are for, and their limits", explanation: "Models may answer different questions. Comparing definitions, measures, context, and limits prevents treating them as interchangeable labels." },
      ];
  ...
  return (
    <section id="evaluacion-avanzada" ref={ref} className={`advanced-assessment-section py-20 bg-gradient-to-br from-slate-50 to-blue-50 section-fade ${isVisible ? 'visible' : ''}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Card className="advanced-quiz-card shadow-lg mb-16">
          <CardContent className="p-8">
            {!showResults ? (
              <div>
                <div className="flex items-center justify-between mb-6">
                  <div className="advanced-quiz-progress flex gap-2">
                    {advancedQuestions.map((_, i) => (
                      <div key={i} className={`advanced-quiz-step w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${i === currentTest ? 'is-current bg-blue-600 text-white' : responses[i] !== undefined ? 'is-complete bg-green-500 text-white' : 'bg-gray-200 text-gray-600'}`}>{i + 1}</div>
                    ))}
                  </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {currentQ.options.map((option, i) => (
                      <button key={i} onClick={() => handleAnswer(option)} className={`advanced-quiz-option p-4 border-2 rounded-xl text-left text-gray-900 font-medium transition-all duration-200 ${responses[currentTest] === option ? 'is-selected border-blue-600 bg-blue-50 text-blue-900' : 'border-gray-200 hover:border-blue-400 hover:bg-blue-50'}`}>{option}</button>
                    ))}
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="space-y-4 mb-8">
                  {advancedQuestions.map((q, i) => {
                    ...
                    return (
                      <div key={i} className={`advanced-quiz-result border-2 rounded-xl p-4 ${isCorrect ? 'is-correct border-green-400 bg-green-50' : 'is-review border-red-300 bg-red-50'}`}>
